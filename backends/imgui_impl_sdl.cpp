...
//  ImGuiPlatformIO interface. 2022-02-04: Added SDL_Renderer* parameter to
//  ImGui_ImplSDL2_InitForSDLRenderer(), so we can use
//  SDL_GetRendererOutputSize() instead of SDL_GL_GetDrawableSize() when bound
//  to a SDL_Renderer. 2022-01-26: Inputs: replaced short-lived
...
bool ImGui_ImplSDL2_ProcessEvent(const SDL_Event *event) {
  ...
  case SDL_MOUSEWHEEL: {
    float wheel_x = (event->wheel.x > 0)   ? 1.0f
                    : (event->wheel.x < 0) ? -1.0f
                                           : 0.0f;
    float wheel_y = (event->wheel.y > 0)   ? 1.0f
                    : (event->wheel.y < 0) ? -1.0f
                                           : 0.0f;
  ...
  case SDL_WINDOWEVENT: {
    ...
    // event on every mouse move, but the final ENTER tends to be right. However
    // we won't get a correct LEAVE event for a captured window.
...
static bool ImGui_ImplSDL2_Init(SDL_Window *window, SDL_Renderer *renderer,
  ...
  // https://github.com/ocornut/imgui/issues/4960)
#ifndef __APPLE__
  if (mouse_can_use_global_state)
    io.BackendFlags |=
  ...
  // doesn't emit the event. Without this, when clicking to gain focus, our
  // widgets wouldn't activate even though they showed as hovered. (This is
  // unfortunately a global SDL setting, so enabling it might have a side-effect
  // on your application. It is unlikely to make a difference, but if your app
  // absolutely needs to ignore the initial on-focus click: you can ignore
  // SDL_MOUSEBUTTONDOWN events coming right after a
...
static void ImGui_ImplSDL2_UpdateMouseCursor() {
  ...
  if ((SDL_GetWindowFlags(bd->Window) & SDL_WINDOW_MOUSE_FOCUS) == 0)
    return;
  ...
  if (io.MouseDrawCursor || imgui_cursor == ImGuiMouseCursor_None) {
    ...
    SDL_SetCursor(bd->MouseCursors[imgui_cursor]
...
// FIXME-PLATFORM: SDL doesn't have an event to notify the application of
