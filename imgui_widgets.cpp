...
bool ImGui::ImageSizeButtonWithText(ImTextureID texId, float button_width,
  ...
  const ImRect bb(window->DC.CursorPos,
                  window->DC.CursorPos + totalSize + padding * 2);
  ImVec2 start(0, 0);
  ...
  // Render
  const ImU32 col = ImGui::GetColorU32((hovered && held) ? ImGuiCol_ButtonActive
                                       : hovered ? ImGuiCol_ButtonHovered
                                                 : ImGuiCol_Button);
  ImGui::RenderFrame(
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2 &pos,
  ...
  ImVec2 center = bb.GetCenter();
  if (hovered || held)
    window->DrawList->AddCircleFilled(center + ImVec2(0, -0.5f),
                                      g.FontSize * 0.5f + 1.0f, bg_col, 12);

  if (dock_node)
    RenderArrowDockMenu(window->DrawList, bb.Min + g.Style.FramePadding,
                        g.FontSize, text_col);
  else
...
void ImGui::AlignTextToFramePadding(float offsetY) {
  ...
  window->DC.CurrLineSize.y = ImMax(window->DC.CurrLineSize.y,
                              ...
                              offsetY;
  window->DC.CurrLineTextBaseOffset = ImMax(window->DC.CurrLineTextBaseOffset,
                                            g.Style.FramePadding.y - offsetY);
}
...
bool ImGui::BeginCombo(const char *label,
                       const std::function<void()> &preview_render,
  ...
  if (!(flags & ImGuiComboFlags_NoPreview)) {
    ...
    ImVec2 preview_size =
        ImVec2(value_x2 - preview_pos.x, bb.Max.y - preview_pos.y);
...
bool ImGui::BeginComboPreview() {
  ...
  if (window->SkipItems ||
      !window->ClipRect.Overlaps(
          g.LastItemData.Rect)) // FIXME: Because we don't have a
...
static void SanitizeFormatString(const char *fmt, char *fmt_out,
                                 size_t fmt_out_size) {
  IM_UNUSED(fmt_out_size);
  const char *fmt_end = ImParseFormatFindEnd(fmt);
  IM_ASSERT(
      (size_t)(fmt_end - fmt + 1) <
...
bool ImGui::DragScalar(const char *label, ImGuiDataType data_type, void *p_data,
  ...
  if (!temp_input_is_active) {
    ...
    if (input_requested_by_tabbing || clicked || double_clicked ||
...
bool ImGui::DragScalar(const char *label, ImGuiDataType data_type, void *p_data,
  ...
  if (power != 1.0f) {
    ...
    IM_ASSERT(p_min != NULL &&
              p_max != NULL); // When using a power curve the drag needs to have
                              // known bounds
    drag_flags |=
        ImGuiSliderFlags_Logarithmic; // Fallback for non-asserting paths
  }
  return DragScalar(label, data_type, p_data, v_speed, p_min, p_max, format,
                    drag_flags);
}
...
bool ImGui::SliderBehaviorT(const ImRect &bb, ImGuiID id,
  ...
  if (g.ActiveId == id) {
    ...
    if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
      ...
      float input_delta =
          (axis == ImGuiAxis_X) ? input_delta2.x : -input_delta2.y;
      if (input_delta != 0.0f) {
        const int decimal_precision =
            is_floating_point ? ImParseFormatPrecision(format, 3) : 0;
        if (decimal_precision > 0) {
...
bool ImGui::SliderScalar(const char *label, ImGuiDataType data_type,
  ...
  if (!temp_input_is_active) {
    ...
    if (input_requested_by_tabbing || clicked || g.NavActivateId == id ||
      ...
      g.ActiveIdUsingNavDirMask |= (1 << ImGuiDir_Left) | (1 << ImGuiDir_Right);
      if (temp_input_allowed &&
          (input_requested_by_tabbing || (clicked && g.IO.KeyCtrl) ||
           g.NavActivateInputId == id))
        temp_input_is_active = true;
    }
  }
...
bool ImGui::SliderScalarN(const char *label, ImGuiDataType data_type, void *v,
  ...
  return SliderScalarN(label, data_type, v, components, v_min, v_max, format,
                       slider_flags);
}
...
bool ImGui::InputScalar(const char *label, ImGuiDataType data_type,
  ...
  flags |=
      ImGuiInputTextFlags_NoMarkEdited; // We call MarkItemEdited() ourselves by
...
bool ImGui::InputColorScalar(const char *label, ImGuiDataType data_type,
                             ImVec4 color, void *p_data, const void *p_step,
                             const void *p_step_fast, const char *format,
                             ImGuiInputTextFlags flags) {
  ImGuiWindow *window = GetCurrentWindow();
  if (window->SkipItems)
  ...
  draw_list->AddRectFilled(
      ImVec2(pos.x, pos.y),
      ImVec2(pos.x + colorBoxWidth, pos.y + ImGui::GetTextLineHeight()),
      ImColor(color));
...
bool ImGui::InputColorFloat(const char *label, float *v, ImVec4 color,
                            float step, float step_fast, const char *format,
                            ImGuiInputTextFlags flags) {
  flags |= ImGuiInputTextFlags_CharsScientific;
  return InputColorScalar(label, ImGuiDataType_Float, color, (void *)v,
                          (void *)(step > 0.0f ? &step : NULL),
...
bool ImGui::InputTextWithImage(const char *label, ImTextureID texture,
  ...
  IM_ASSERT(
      !(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
  return InputTextEx(label, texture, NULL, buf, (int)buf_size, ImVec2(0, 0),
...
bool ImGui::InputTextEx(const char *label, const char *hint, char *buf,
  ...
  const bool input_requested_by_nav =
      (g.ActiveId != id) && ((g.NavActivateInputId == id) ||
                             (g.NavActivateId == id &&
  ...
  if (g.ActiveId == id) {
    ...
    if ((flags & ImGuiInputTextFlags_AllowTabInput) &&
        IsKeyPressed(ImGuiKey_Tab) && !ignore_char_inputs && !io.KeyShift &&
        !is_readonly)
  ...
  if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
    ...
    const bool is_cut =
        ((is_shortcut_key && IsKeyPressed(ImGuiKey_X)) ||
         (is_shift_key_only && IsKeyPressed(ImGuiKey_Delete))) &&
        !is_readonly && !is_password &&
        (!is_multiline || state->HasSelection());
    const bool is_copy =
        ((is_shortcut_key && IsKeyPressed(ImGuiKey_C)) ||
         (is_ctrl_key_only && IsKeyPressed(ImGuiKey_Insert))) &&
        !is_password && (!is_multiline || state->HasSelection());
    const bool is_paste =
        ((is_shortcut_key && IsKeyPressed(ImGuiKey_V)) ||
         (is_shift_key_only && IsKeyPressed(ImGuiKey_Insert))) &&
        !is_readonly;
    const bool is_undo = ((is_shortcut_key && IsKeyPressed(ImGuiKey_Z)) &&
                          !is_readonly && is_undoable);
    const bool is_redo =
        ((is_shortcut_key && IsKeyPressed(ImGuiKey_Y)) ||
         (is_osx_shift_shortcut && IsKeyPressed(ImGuiKey_Z))) &&
        !is_readonly && is_undoable;
  ...
  if (render_cursor || render_selection) {
    ...
    if (render_cursor && state->CursorFollow) {
      ...
      if (!(flags & ImGuiInputTextFlags_NoHorizontalScroll)) {
        ...
        if (cursor_offset.x < state->ScrollX)
          ...
          state->ScrollX =
              IM_FLOOR(cursor_offset.x - visible_width + scroll_increment_x);
      } else {
        state->ScrollX = 0.0f;
      }
    ...
    if (render_cursor) {
      ...
      ImVec2 cursor_screen_pos =
          ImFloor(draw_pos + cursor_offset - draw_scroll);
...
bool ImGui::InputTextEx(const char *label, ImTextureID texture,
  ...
  if (render_cursor || render_selection) {
    ...
    if (is_multiline ||
      ...
      draw_window->DrawList->AddText(
          g.Font, g.FontSize,
          ImVec2(draw_pos.x + icon_size, draw_pos.y) - draw_scroll, col,
          buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
    }
    ...
    if (is_multiline ||
      ...
      ImU32 col = GetColorU32(is_displaying_hint ? ImGuiCol_TextDisabled
                                                 : ImGuiCol_Text);
      draw_window->DrawList->AddText(
          g.Font, g.FontSize, ImVec2(draw_pos.x + icon_size, draw_pos.y), col,
          buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
    }
  }
...
void ImGui::ColorTooltip(const char *text, const float *col,
  ...
  BeginTooltipEx(ImGuiTooltipFlags_OverridePreviousTooltip,
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImTextureID *texture,
                             ImGuiTreeNodeFlags flags, const char *label,
                             const char *label_end) {
  ImGuiWindow *window = GetCurrentWindow();
  if (window->SkipItems)
    return false;
  ...
  if (display_frame) {
    ...
    if (flags & ImGuiTreeNodeFlags_Bullet)
      ...
      RenderArrow(window->DrawList, texture,
...
// ImGuiTreeNodeFlags_CollapsingHeader). You can remove the _NoTreePushOnOpen
// flag if you want behavior closer to normal TreeNode().
bool ImGui::CollapsingHeader(const char *label, ImTextureID *texture,
                             ImGuiTreeNodeFlags flags) {
  ImGuiWindow *window = GetCurrentWindow();
  if (window->SkipItems)
    return false;

  return TreeNodeBehavior(window->GetID(label), texture,
                          flags | ImGuiTreeNodeFlags_CollapsingHeader, label);
}
...
bool ImGui::BeginBottomBar() {
  ...
  // analogous here, maybe a BeginGroupEx() with flags).
  window->DC.CursorPos = window->DC.CursorMaxPos =
      ImVec2(bar_rect.Min.x + window->DC.BottomBarOffset.x,
             bar_rect.Min.y + window->DC.BottomBarOffset.y);
  window->DC.LayoutType = ImGuiLayoutType_Horizontal;
  window->DC.NavLayerCurrent = ImGuiNavLayer_Menu;
...
static bool IsRootOfOpenMenuSet() {
  ...
  return (/*upper_popup->OpenParentId == window->IDStack.back() &&*/ upper_popup
              ->Window &&
          (upper_popup->Window->Flags & ImGuiWindowFlags_ChildMenu));
}

bool ImGui::BeginMenuImageEx(const char *label, ImTextureID *texture,
                             const char *icon, bool enabled) {
  ImGuiWindow *window = GetCurrentWindow();
  if (window->SkipItems)
    return false;
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    ImVec2 icon_pos(window->DC.CursorPos.x,
                    window->DC.CursorPos.y + window->DC.CurrLineTextBaseOffset);
    RenderText(text_pos, label);
    if (icon && icon[0])
      window->DrawList->AddImage(*texture, ImVec2(icon_pos.x, icon_pos.y),
                                 ImVec2(icon_pos.x + style.FramePadding.y,
    ...
    float icon_w = (icon && icon[0])
                       ? CalcTextSize(icon, NULL).x + style.FramePadding.y
                       : 0.0f;
    ...
    pressed = Selectable("", menu_is_open,
                         ...
                         ImVec2(min_w, 0.0f));
    RenderArrow(window->DrawList,
                pos + ImVec2(style.ItemInnerSpacing.x + icon_w + extra_w +
                                 g.FontSize * 0.30f,
                             0.0f),
...
bool ImGui::BeginMenuImage(const char *label, ImTextureID *texture,
                           bool enabled) {
...
bool ImGui::MenuItemImageEx(const char *label, ImTextureID texture,
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    float checkmark_w = IM_FLOOR(g.FontSize * 1.20f);
    float min_w = window->DC.MenuColumns.DeclColumns(icon_w, label_size.x,
                                                     shortcut_w, checkmark_w);
    ...
    if (texture != NULL) {
      ...
      window->DrawList->AddImage(texture, texture_pos,
...
bool ImGui::MenuItem(const char *label, const char *shortcut,
                     ImTextureID texture, bool selected, bool enabled) {
...
bool ImGui::BeginTabBarEx(ImGuiTabBar *tab_bar, const ImRect &tab_bar_bb,
                          ImGuiTabBarFlags flags, ImGuiDockNode *dock_node) {
  ImGuiContext &g = *GImGui;
  ImGuiWindow *window = g.CurrentWindow;
  if (window->SkipItems)
    return false;
  ...
  if (dock_node != NULL) {
    ...
    const float separator_max_x =
        dock_node->Pos.x + dock_node->Size.x - window->WindowBorderSize;
...
static void ImGui::TabBarScrollToTab(ImGuiTabBar *tab_bar, ImGuiID tab_id,
  ...
  float scrollable_width = tab_bar->BarRect.GetWidth() - sections[0].Width -
...
bool ImGui::TabItemEx(ImGuiTabBar *tab_bar, const char *label, bool *p_open,
  ...
  if (tab->Window != NULL) {
    ...
    tab_bar->TabsNames.append(label,
                              label + strlen(label) +
                                  1); // Append name _with_ the zero-terminator.
  }
      ...
      if (!undocking_tab) //&& (!g.IO.ConfigDockingWithShift || g.IO.KeyShift)
      {
        ...
        if (drag_distance_from_edge_x > threshold_x)
          if ((drag_dir < 0 && tab_bar->GetTabOrder(tab) == 0) ||
              (drag_dir > 0 &&
               tab_bar->GetTabOrder(tab) == tab_bar->Tabs.Size - 1))
            undocking_tab = true;
      }
  ...
  if (targetWindow) {
    if (targetWindow->textureID == NULL) {
      ...
      TabItemLabelAndCloseButton(display_draw_list, targetWindow->textureID, bb,
                                 flags, tab_bar->FramePadding, label, id,
...
void ImGui::TabItemBackground(ImDrawList *draw_list, const ImRect &bb,
  ...
  const float y2 =
      bb.Max.y + ((flags & ImGuiTabItemFlags_Preview) ? 0.0f : -1.0f);
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList *draw_list, const ImRect &bb,
  ...
  const ImVec2 button_pos(
      ImMax(bb.Min.x, bb.Max.x - frame_padding.x * 2.0f - button_sz), bb.Min.y);
  ...
  if (close_button_visible) {
    ...
    const ImRect bullet_bb(button_pos, button_pos +
                                           ImVec2(button_sz, button_sz) +
                                           g.Style.FramePadding * 2.0f);
...
void ImGui::TabItemLabelAndCloseButton(
    ImDrawList *draw_list, ImTextureID texture, const ImRect &bb,
    ImGuiTabItemFlags flags, ImVec2 frame_padding, const char *label,
    ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible,
  ...
  if (texture != NULL) {
    ...
    ImVec2 texture_pos(bb.Min.x + frame_padding.x,
                       bb.Min.y + (bb.GetHeight() - texture_height) *
                                      0.5f); // Adjust position as needed
    draw_list->AddImage(
        texture, texture_pos,
        ImVec2(texture_pos.x + texture_width, texture_pos.y + texture_height));
  }
  ...
  ImRect text_pixel_clip_bb(
      ...
      bb.Min.y + frame_padding.y, bb.Max.x - frame_padding.x, bb.Max.y);
  ImRect text_ellipsis_clip_bb = text_pixel_clip_bb;
