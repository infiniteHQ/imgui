...
#pragma warning(disable : 4996)            // 'This function or variable may be unsafe': strcpy,
...
#pragma warning(disable : 26812)  // [Static Analyzer] The enum type 'xxx' is unscoped.
...
#pragma clang diagnostic ignored "-Wunknown-warning-option"  // warning: unknown warning group 'xxx' // not
...
#pragma clang diagnostic ignored "-Wzero-as-null-pointer-constant"    // warning: zero as null pointer constant
                                                                      ...
                                                                      // #define NULL 0
#pragma clang diagnostic ignored "-Wdouble-promotion"                 // warning: implicit conversion from 'float' to
                                                                      // 'double' when passing argument to function  // using
                                                                      // printf() is a misery with this as C++ va_arg
                                                                      // ellipsis changes float to double.
#pragma clang diagnostic ignored "-Wenum-enum-conversion"             // warning: bitwise operation between different
                                                                      // enumeration types ('XXXFlags_' and
                                                                      // 'XXXFlagsPrivate_')
#pragma clang diagnostic ignored "-Wdeprecated-enum-enum-conversion"  // warning: bitwise operation between
...
#pragma GCC diagnostic ignored "-Wformat-nonliteral"  // warning: format not a string literal, format string
                                                      // not checked
#pragma GCC diagnostic ignored "-Wclass-memaccess"    // [__GNUC__ >= 8] warning: 'memset/memcpy'
...
static const float DRAG_MOUSE_THRESHOLD_FACTOR = 0.50f;  // Multiplier for the default value of io.MouseDragThreshold to make
...
void ImGui::LabelTextV(const char *label, const char *fmt, va_list args) {
  ...
  // Render
  RenderTextClipped(
      value_bb.Min + style.FramePadding, value_bb.Max, value_text_begin, value_text_end, &value_size, ImVec2(0.0f, 0.0f));
...
bool ImGui::ButtonBehavior(const ImRect &bb, ImGuiID id, bool *out_hovered, bool *out_held, ImGuiButtonFlags flags) {
    ...
    if (!(flags & ImGuiButtonFlags_NoKeyModifiers) || (!g.IO.KeyCtrl && !g.IO.KeyShift && !g.IO.KeyAlt)) {
      ...
      if ((flags & ImGuiButtonFlags_PressedOnRelease) && mouse_button_released != -1) {
        ...
        const bool has_repeated_at_least_once =
            (flags & ImGuiButtonFlags_Repeat) && g.IO.MouseDownDurationPrev[mouse_button_released] >= g.IO.KeyRepeatDelay;
  ...
  if (g.NavActivateDownId == id) {
    ...
    bool nav_activated_by_inputs = IsNavInputTest(
        ImGuiNavInput_Activate, (flags & ImGuiButtonFlags_Repeat) ? ImGuiInputReadMode_Repeat : ImGuiInputReadMode_Pressed);
      ...
      if (g.IO.MouseDown[mouse_button]) {
        ...
        if ((release_in || release_anywhere) && !g.DragDropActive) {
          ...
          bool is_double_click_release = (flags & ImGuiButtonFlags_PressedOnDoubleClick) &&
                                         g.IO.MouseReleased[mouse_button] && g.IO.MouseClickedLastCount[mouse_button] == 2;
...
bool ImGui::ButtonEx(const char *label, const ImVec2 &size_arg, ImGuiButtonFlags flags) {
  ...
  if ((flags & ImGuiButtonFlags_AlignTextBaseLine) &&
      style.FramePadding.y < window->DC.CurrLineTextBaseOffset)  // Try to vertically align buttons that
...
bool ImGui::ImageSizeButtonWithText(
    ImTextureID texId,
    float button_width,
    const char *label,
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2 &pos) {
  ...
  // isn't the regular behavior of buttons, but it doesn't affect the user much
  // because navigation tends to keep items visible).
  ...
  window->DrawList->AddLine(
      center + ImVec2(+cross_extent, +cross_extent), center + ImVec2(-cross_extent, -cross_extent), cross_col, 1.0f);
  window->DrawList->AddLine(
      center + ImVec2(+cross_extent, -cross_extent), center + ImVec2(-cross_extent, +cross_extent), cross_col, 1.0f);
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2 &pos, ImGuiDockNode *dock_node) {
  ...
  if (dock_node)
    ...
    RenderArrow(
        window->DrawList, bb.Min + g.Style.FramePadding, text_col, window->Collapsed ? ImGuiDir_Right : ImGuiDir_Down, 1.0f);
...
bool ImGui::Checkbox(const char *label, bool *v) {
  ...
  if (!ItemAdd(total_bb, id)) {
    IMGUI_TEST_ENGINE_ITEM_INFO(
        id, label, g.LastItemData.StatusFlags | ImGuiItemStatusFlags_Checkable | (*v ? ImGuiItemStatusFlags_Checked : 0));
    return false;
  }
...
bool ImGui::RadioButton(const char *label, bool active) {
  ...
  if (style.FrameBorderSize > 0.0f) {
    window->DrawList->AddCircle(
        center + ImVec2(1, 1), radius, GetColorU32(ImGuiCol_BorderShadow), 16, style.FrameBorderSize);
...
void ImGui::ProgressBar(float fraction, const ImVec2 &size_arg, const char *overlay) {
  ...
  if (overlay_size.x > 0.0f)
    RenderTextClipped(
        ImVec2(
            ImClamp(fill_br.x + style.ItemSpacing.x, bb.Min.x, bb.Max.x - overlay_size.x - style.ItemInnerSpacing.x),
            bb.Min.y),
        bb.Max,
        overlay,
        NULL,
...
void ImGui::NewLine() {
  ...
  if (window->DC.CurrLineSize.y > 0.0f)  // In the event that we are on a line with items that is smaller
...
void ImGui::AlignTextToFramePadding(float offsetY) {
  ImGuiWindow *window = GetCurrentWindow();
  if (window->SkipItems)
    return;

  ImGuiContext &g = *GImGui;
  window->DC.CurrLineSize.y = ImMax(window->DC.CurrLineSize.y, g.FontSize + g.Style.FramePadding.y * 2) - offsetY;
  window->DC.CurrLineTextBaseOffset = ImMax(window->DC.CurrLineTextBaseOffset, g.Style.FramePadding.y - offsetY);
}
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags) {
  ...
  IM_ASSERT(ImIsPowerOfTwo(
      flags & (ImGuiSeparatorFlags_Horizontal | ImGuiSeparatorFlags_Vertical)));  // Check that only 1 option is selected
  ...
  if (flags & ImGuiSeparatorFlags_Vertical) {
    ...
    // FIXME-WORKRECT: old hack (#205) until we decide of consistent behavior
    // with WorkRect/Indent and Separator
    if (g.GroupStack.Size > 0 && g.GroupStack.back().WindowID == window->ID)
      x1 += window->DC.Indent.x;

    // FIXME-WORKRECT: In theory we should simply be using WorkRect.Min.x/Max.x
    // everywhere but it isn't aesthetically what we want, need to introduce a
    // variant of WorkRect for that purpose. (#4787)
...
void ImGui::Separator() {
  ...
  flags |= ImGuiSeparatorFlags_SpanAllColumns;  // NB: this only applies to
                                                // legacy Columns() api as they
                                                // relied on Separator() a lot.
  SeparatorEx(flags);
}
...
bool ImGui::SplitterBehavior(
  ...
  if (hovered)
    g.LastItemData.StatusFlags |= ImGuiItemStatusFlags_HoveredRect;  // for IsItemHovered(), because
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect &bb, ImGuiComboFlags flags) {
  ...
  if (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSizeConstraint) {
    ...
    SetNextWindowSizeConstraints(
        ImVec2(w, 0.0f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)));
  }
  ...
  if (ImGuiWindow *popup_window = FindWindowByName(name))
    if (popup_window->WasActive) {
      ...
      popup_window->AutoPosLastDirection =
          (flags & ImGuiComboFlags_PopupAlignLeft) ? ImGuiDir_Left : ImGuiDir_Down;  // Left = "Below, Toward Left", Down =
                                                                                     // "Below, Toward Right (default)"
      ImRect r_outer = GetPopupAllowedExtentRect(popup_window);
      ImVec2 pos = FindBestWindowPosForPopupEx(
          bb.GetBL(), size_expected, &popup_window->AutoPosLastDirection, r_outer, bb, ImGuiPopupPositionPolicy_ComboBox);
...
bool ImGui::BeginComboPreview() {
  ...
  if (window->SkipItems || !window->ClipRect.Overlaps(g.LastItemData.Rect))  // FIXME: Because we don't have a
                                                                             // ImGuiItemStatusFlags_Visible flag to test
                                                                             // last ItemAdd() result
    return false;
  IM_ASSERT(
      g.LastItemData.Rect.Min.x == preview_data->PreviewRect.Min.x &&
      g.LastItemData.Rect.Min.y == preview_data->PreviewRect.Min.y);  // Didn't call after BeginCombo/EndCombo block or
...
bool ImGui::Combo(const char *label, int *current_item, const char *items_separated_by_zeros, int height_in_items) {
  int items_count = 0;
  const char *p = items_separated_by_zeros;  // FIXME-OPT: Avoid computing this, or at least
...
static const char *PatchFormatStringFloatToInt(const char *fmt) {
  ...
  const char *fmt_end = ImParseFormatFindEnd(fmt_start);  // Find end of format specifier, which itself is an exercise
  ...
  if (fmt_end > fmt_start && fmt_end[-1] == 'f') {
    ...
    return g.TempBuffer;
#else
    IM_ASSERT(0 && "DragInt(): Invalid format string!");  // Old versions used a default
...
bool ImGui::DataTypeApplyFromText(const char *buf, ImGuiDataType data_type, void *p_data, const char *format) {
  ...
  if (data_type == ImGuiDataType_S32 || data_type == ImGuiDataType_U32 || data_type == ImGuiDataType_S64 ||
      data_type == ImGuiDataType_U64 || data_type == ImGuiDataType_Float || data_type == ImGuiDataType_Double) {
...
static void SanitizeFormatString(const char *fmt, char *fmt_out, size_t fmt_out_size) {
  ...
  while (fmt < fmt_end) {
    char c = *(fmt++);
    if (c != '\'' && c != '$' && c != '_')  // Custom flags provided by stb_sprintf.h. POSIX 2008 also
                                            // supports '.
      *(fmt_out++) = c;
  }
...
bool ImGui::DragScalar(
  ...
  else if (data_type == ImGuiDataType_S32 && strcmp(format, "%d") != 0)  // (FIXME-LEGACY: Patch old "%.0f" format string to
                                                                         // use "%d", read function more details.)
...
bool ImGui::DragFloatRange2(
  ...
  value_changed |= DragScalar(
      "##max", ImGuiDataType_Float, v_current_max, v_speed, &max_min, &max_max, format_max ? format_max : format, max_flags);
  PopItemWidth();
  SameLine(0, g.Style.ItemInnerSpacing.x);
...
bool ImGui::DragScalar(
  ...
  if (power != 1.0f) {
    ...
    IM_ASSERT(p_min != NULL && p_max != NULL);   // When using a power curve the drag needs to have
                                                 // known bounds
  ...
  if (is_logarithmic) {
    ...
    else if ((v_min * v_max) < 0.0f)  // Range crosses zero, so split into two portions
    {
      float zero_point_center =
          (-(float)v_min) / ((float)v_max - (float)v_min);  // The zero point in parametric space.  There's an
...
TYPE ImGui::ScaleValueFromRatioT(
  ...
  if (is_logarithmic) {
    ...
    // "mathematically correct" but non-intuitive behaviors like a fully-left
    // slider not actually reaching the minimum value
    if (t <= 0.0f)
      result = v_min;
    else if (t >= 1.0f)
      result = v_max;
      ...
      float t_with_flip = flipped ? (1.0f - t) : t;  // t, but flipped if necessary to account
                                                     // for us flipping the range

      if ((v_min * v_max) < 0.0f)  // Range crosses zero, so we have to do this in two parts
      {
        float zero_point_center =
            (-(float)ImMin(v_min, v_max)) / ImAbs((float)v_max - (float)v_min);  // The zero point in parametric space
        float zero_point_snap_L = zero_point_center - zero_deadzone_halfsize;
        float zero_point_snap_R = zero_point_center + zero_deadzone_halfsize;
  ...
  if (g.ActiveId == id) {
    ...
    if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
      ...
      const ImVec2 input_delta2 = GetNavInputAmount2d(
          ImGuiNavDirSourceFlags_Keyboard | ImGuiNavDirSourceFlags_PadDPad, ImGuiInputReadMode_RepeatFast, 0.0f, 0.0f);
      ...
      if (input_delta != 0.0f) {
        ...
        if (decimal_precision > 0) {
          ...
          if ((v_range >= -100.0f && v_range <= 100.0f) || IsNavInputDown(ImGuiNavInput_TweakSlow))
            input_delta =
                ((input_delta < 0.0f) ? -1.0f : +1.0f) / (float)v_range;  // Gamepad/keyboard tweak speeds in integer steps
          else
            input_delta /= 100.0f;
        }
      ...
      if (g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated) {
        ...
        if ((clicked_t >= 1.0f && delta > 0.0f) ||
            (clicked_t <= 0.0f && delta < 0.0f))  // This is to avoid applying the saturation when
  ...
  switch (data_type) {
    case ImGuiDataType_S8: {
      ...
      bool r = SliderBehaviorT<ImS32, ImS32, float>(
          bb, id, ImGuiDataType_S32, &v32, *(const ImS8 *)p_min, *(const ImS8 *)p_max, format, flags, out_grab_bb);
      if (r)
        *(ImS8 *)p_v = (ImS8)v32;
      return r;
    }
    case ImGuiDataType_U8: {
      ...
      bool r = SliderBehaviorT<ImU32, ImS32, float>(
          bb, id, ImGuiDataType_U32, &v32, *(const ImU8 *)p_min, *(const ImU8 *)p_max, format, flags, out_grab_bb);
      if (r)
        *(ImU8 *)p_v = (ImU8)v32;
      return r;
    }
    case ImGuiDataType_S16: {
      ImS32 v32 = (ImS32) * (ImS16 *)p_v;
      bool r = SliderBehaviorT<ImS32, ImS32, float>(
          bb, id, ImGuiDataType_S32, &v32, *(const ImS16 *)p_min, *(const ImS16 *)p_max, format, flags, out_grab_bb);
      if (r)
        *(ImS16 *)p_v = (ImS16)v32;
      return r;
    }
    case ImGuiDataType_U16: {
      ImU32 v32 = (ImU32) * (ImU16 *)p_v;
      bool r = SliderBehaviorT<ImU32, ImS32, float>(
          bb, id, ImGuiDataType_U32, &v32, *(const ImU16 *)p_min, *(const ImU16 *)p_max, format, flags, out_grab_bb);
      if (r)
        *(ImU16 *)p_v = (ImU16)v32;
      return r;
    }
    case ImGuiDataType_S32:
      ...
      return SliderBehaviorT<ImS32, ImS32, float>(
          bb, id, data_type, (ImS32 *)p_v, *(const ImS32 *)p_min, *(const ImS32 *)p_max, format, flags, out_grab_bb);
    case ImGuiDataType_U32:
      IM_ASSERT(*(const ImU32 *)p_max <= IM_U32_MAX / 2);
      return SliderBehaviorT<ImU32, ImS32, float>(
          bb, id, data_type, (ImU32 *)p_v, *(const ImU32 *)p_min, *(const ImU32 *)p_max, format, flags, out_grab_bb);
    case ImGuiDataType_S64:
      IM_ASSERT(*(const ImS64 *)p_min >= IM_S64_MIN / 2 && *(const ImS64 *)p_max <= IM_S64_MAX / 2);
      return SliderBehaviorT<ImS64, ImS64, double>(
          bb, id, data_type, (ImS64 *)p_v, *(const ImS64 *)p_min, *(const ImS64 *)p_max, format, flags, out_grab_bb);
    case ImGuiDataType_U64:
      IM_ASSERT(*(const ImU64 *)p_max <= IM_U64_MAX / 2);
      return SliderBehaviorT<ImU64, ImS64, double>(
          bb, id, data_type, (ImU64 *)p_v, *(const ImU64 *)p_min, *(const ImU64 *)p_max, format, flags, out_grab_bb);
    case ImGuiDataType_Float:
      IM_ASSERT(*(const float *)p_min >= -FLT_MAX / 2.0f && *(const float *)p_max <= FLT_MAX / 2.0f);
      return SliderBehaviorT<float, float, float>(
          bb, id, data_type, (float *)p_v, *(const float *)p_min, *(const float *)p_max, format, flags, out_grab_bb);
    case ImGuiDataType_Double:
...
bool ImGui::VSliderScalar(
  ...
  const bool value_changed =
      SliderBehavior(frame_bb, id, data_type, p_data, p_min, p_max, format, flags | ImGuiSliderFlags_Vertical, &grab_bb);
  if (value_changed)
    MarkItemEdited(id);
...
bool ImGui::InputScalar(
  ...
  flags |= ImGuiInputTextFlags_NoMarkEdited;  // We call MarkItemEdited() ourselves by
                                              // comparing the actual data rather than
                                              // the string.
...
bool ImGui::InputColorScalar(
  ...
  // Set the width of the color box
  const float colorBoxWidth = 20.0f;  // Width of the color box
  const float spacing = 5.0f;         // Space between color box and input

  BeginGroup();  // The only purpose of the group here is to allow the caller to
                 // query item data e.g. IsItemActive()
  PushID(label);

  // Calculate width for the input field
  float inputWidth = ImMax(1.0f, CalcItemWidth() - (colorBoxWidth + spacing));
  ...
  draw_list->AddRectFilled(
      ImVec2(pos.x, pos.y), ImVec2(pos.x + colorBoxWidth, pos.y + ImGui::GetTextLineHeight()), ImColor(color));

  // Move cursor position to the right of the color box
...
bool ImGui::InputTextWithImage(
    ...
    void *user_data) {
  IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline));  // call InputTextMultiline()
  return InputTextEx(label, texture, NULL, buf, (int)buf_size, ImVec2(0, 0), flags, callback, user_data);
}
...
#include "imstb_textedit.h"
  ...
  static void
  stb_textedit_replace(ImGuiInputTextState *str, STB_TexteditState *state, const STB_TEXTEDIT_CHARTYPE *text, int text_len) {
...
bool ImGui::InputTextEx(
  ...
  if (is_multiline) {
    ...
    // FIXME-NAV: Pressing NavActivate will trigger general child activation
    // right before triggering our own below. Harmless but bizarre.
    PushStyleColor(ImGuiCol_ChildBg, style.Colors[ImGuiCol_FrameBg]);
    PushStyleVar(ImGuiStyleVar_ChildRounding, style.FrameRounding);
    PushStyleVar(ImGuiStyleVar_ChildBorderSize, style.FrameBorderSize);
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0, 0));  // Ensure no clip rect so mouse hover can reach
                                                              // FramePadding edges
    ...
    draw_window->DC.NavLayersActiveMaskNext |=
        (1 << draw_window->DC.NavLayerCurrent);  // This is to ensure that EndChild() will
  ...
  if ((init_state && g.ActiveId != id) || init_changed_specs) {
    ...
    state->TextW.resize(buf_size + 1);  // wchar count <= UTF-8 count. we use +1 to make sure that .Data is
                                        // always pointing to at least an empty string.
    state->TextA.resize(0);
    state->TextAIsValid = false;  // TextA is not valid yet (we will display buf until then)
    state->CurLenW = ImTextStrFromUtf8(state->TextW.Data, buf_size, buf, NULL, &buf_end);
    state->CurLenA = (int)(buf_end - buf);  // We can't get the result from ImStrncpy() above because it
  ...
  if (g.ActiveId != id && init_make_active) {
    ...
    if (flags &
        (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_AllowTabInput))  // Disable keyboard tabbing out as
  ...
  if (g.ActiveId == id) {
    ...
    // It is ill-defined whether the backend needs to send a \t character when
  ...
  if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
    ...
    const bool is_wordmove_key_down = is_osx ? io.KeyAlt : io.KeyCtrl;  // OS X style: Text editing cursor
                                                                        // movement using Alt instead of Ctrl
    const bool is_startend_key_down =
        is_osx && io.KeySuper && !io.KeyCtrl && !io.KeyAlt;  // OS X style: Line/Text Start and End using Cmd+Arrows
                                                             // instead of Home/End
    ...
    const bool is_validate_nav =
        (IsNavInputTest(ImGuiNavInput_Activate, ImGuiInputReadMode_Pressed) && !IsKeyPressed(ImGuiKey_Space)) ||
    ...
    if (apply_edit_back_to_user_buffer) {
      ...
      if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory |
        ...
        if (event_flag) {
          ...
          callback_buf = is_readonly ? buf : state->TextA.Data;  // Pointer may have been invalidated by a
                                                                 // resize callback
          ...
          if (buf_dirty) {
            IM_ASSERT(callback_data.BufTextLen == (int)strlen(callback_data.Buf));  // You need to maintain BufTextLen if
                                                                                    // you change the text!
            if (callback_data.BufTextLen > backup_current_text_length && is_resizable)
              state->TextW.resize(state->TextW.Size + (callback_data.BufTextLen - backup_current_text_length));
            state->CurLenW = ImTextStrFromUtf8(state->TextW.Data, state->TextW.Size, callback_data.Buf, NULL);
            state->CurLenA = callback_data.BufTextLen;  // Assume correct length and valid UTF-8 from
  ...
  if (render_cursor || render_selection) {
    ...
    if (render_cursor && state->CursorFollow) {
      ...
      if (is_multiline) {
        ...
        draw_pos.y += (draw_window->Scroll.y - scroll_y);  // Manipulate cursor pos immediately avoid a
                                                           // frame of lag
    ...
    if (render_selection) {
      ...
      ImU32 bg_color = GetColorU32(
          ...
          render_cursor ? 1.0f : 0.6f);                // FIXME: current code flow mandate that
    ...
    if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length) {
      ImU32 col = GetColorU32(is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
      draw_window->DrawList->AddText(
          g.Font, g.FontSize, draw_pos, col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
    }
  }
...
bool ImGui::InputTextEx(
  ...
  const ImVec2 icon_size_vec(icon_size, icon_size);
  ImVec2 icon_pos;
  icon_pos.x = frame_bb.Min.x + 8.0f;
  icon_pos.y = frame_bb.Min.y + (frame_size.y - icon_size) * 0.5f;
  ...
  window->DrawList->AddImage(texture, icon_pos, icon_pos + icon_size_vec);

  ImVec2 text_start_pos = frame_bb.Min;
  text_start_pos.x = icon_pos.x + icon_size + style.ItemInnerSpacing.x;
  SetCursorScreenPos(text_start_pos);
  ...
  if (render_cursor || render_selection) {
...

    {
      ...
      // Itérer sur les lignes pour trouver les positions
      searches_remaining += is_multiline ? 1 : 0;
      int line_count = 0;
      for (const ImWchar *s = text_begin; *s != 0; s++)
        if (*s == '\n') {
      ...
      // Calculer la position du curseur en tenant compte de l'icône
      cursor_offset.x = InputTextCalcTextSizeW(ImStrbolW(searches_input_ptr[0], text_begin), searches_input_ptr[0]).x +
                        (icon_size + style.ItemInnerSpacing.x);
      cursor_offset.y = searches_result_line_no[0] * g.FontSize;
      if (searches_result_line_no[1] >= 0) {
        select_start_offset.x =
            InputTextCalcTextSizeW(ImStrbolW(searches_input_ptr[1], text_begin), searches_input_ptr[1]).x +
    ...
    if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length) {
      ...
      draw_window->DrawList->AddText(
          g.Font,
          g.FontSize,
          ImVec2(draw_pos.x + icon_size, draw_pos.y) - draw_scroll,
          col,
          buf_display,
          buf_display_end,
          0.0f,
    ...
    if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length) {
      ...
      draw_window->DrawList->AddText(
          g.Font,
          g.FontSize,
          ImVec2(draw_pos.x + icon_size, draw_pos.y),
          col,
          buf_display,
          buf_display_end,
          0.0f,
...
static void ColorEditRestoreHS(const float *col, float *H, float *S, float *V) {
  // This check is optional. Suppose we have two color widgets side by side,
  // both widgets display different colors, but both colors have hue and/or
  // saturation undefined. With color check: hue/saturation is preserved in one
  // widget. Editing color in one widget would reset hue/saturation in another
  // one. Without color check: common hue/saturation would be displayed in all
  // widgets that have hue/saturation undefined. g.ColorEditLastColor is stored
  // as ImU32 RGB value: this essentially gives us color equality check with
  // reduced precision. Tiny external color changes would not be detected and
  // this check would still pass. This is OK, since we only restore
  // hue/saturation _only_ if they are undefined, therefore this change flipping
  // hue/saturation from undefined to a very tiny value would still be
...
bool ImGui::ColorEdit4(const char *label, float col[4], ImGuiColorEditFlags flags) {
  ...
  int i[4] = {
    IM_F32_TO_INT8_UNBOUND(f[0]), IM_F32_TO_INT8_UNBOUND(f[1]), IM_F32_TO_INT8_UNBOUND(f[2]), IM_F32_TO_INT8_UNBOUND(f[3])
  };
  ...
  if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 &&
    ...
    if (alpha)
      ...
      ImFormatString(
          buf, IM_ARRAYSIZE(buf), "#%02X%02X%02X", ImClamp(i[0], 0, 255), ImClamp(i[1], 0, 255), ImClamp(i[2], 0, 255));
    SetNextItemWidth(w_inputs);
    if (InputText(
            "##Text", buf, IM_ARRAYSIZE(buf), ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase)) {
      value_changed = true;
      char *p = buf;
  ...
  if (!(flags & ImGuiColorEditFlags_NoSmallPreview)) {
    const float button_offset_x = ((flags & ImGuiColorEditFlags_NoInputs) || (style.ColorButtonPosition == ImGuiDir_Left))
                                      ? 0.0f
    ...
    if (BeginPopup("picker")) {
      ...
      ImGuiColorEditFlags picker_flags = (flags_untouched & picker_flags_to_forward) | ImGuiColorEditFlags_DisplayMask_ |
...
static void RenderArrowsForVerticalBar(ImDrawList *draw_list, ImVec2 pos, ImVec2 half_sz, float bar_w, float alpha) {
  ...
  ImGui::RenderArrowPointingAt(
      ...
      IM_COL32(0, 0, 0, alpha8));
  ImGui::RenderArrowPointingAt(
      draw_list, ImVec2(pos.x + bar_w - half_sz.x, pos.y), half_sz, ImGuiDir_Left, IM_COL32(255, 255, 255, alpha8));
}
...
bool ImGui::ColorPicker4(const char *label, float col[4], ImGuiColorEditFlags flags, const float *ref_col) {
  ...
  if (!(flags & ImGuiColorEditFlags_PickerMask_))
    flags |=
        ((g.ColorEditOptions & ImGuiColorEditFlags_PickerMask_) ? g.ColorEditOptions : ImGuiColorEditFlags_DefaultOptions_) &
        ImGuiColorEditFlags_PickerMask_;
  if (!(flags & ImGuiColorEditFlags_InputMask_))
    flags |=
        ((g.ColorEditOptions & ImGuiColorEditFlags_InputMask_) ? g.ColorEditOptions : ImGuiColorEditFlags_DefaultOptions_) &
  ...
  if (flags & ImGuiColorEditFlags_PickerHueWheel) {
    ...
    if (IsItemActive()) {
      ...
      // Greatly reduces hue jitter and reset to 0 when hue == 255 and color is
  ...
  ImU32 user_col32_striped_of_alpha =
      ColorConvertFloat4ToU32(ImVec4(R, G, B, style.Alpha));  // Important: this is still including the
  ...
  if (flags & ImGuiColorEditFlags_PickerHueWheel) {
    ...
    draw_list->AddRectFilledMultiColor(
        picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), col_white, hue_color32, hue_color32, col_white);
    draw_list->AddRectFilledMultiColor(
    ...
    sv_cursor_pos.y = ImClamp(
        IM_ROUND(picker_pos.y + ImSaturate(1 - V) * sv_picker_size), picker_pos.y + 2, picker_pos.y + sv_picker_size - 2);
...
bool ImGui::ColorButton(const char *desc_id, const ImVec4 &col, ImGuiColorEditFlags flags, ImVec2 size) {
  ...
  if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f) {
    ...
    if (col_source.w < 1.0f)
      RenderColorRectWithAlphaCheckerboard(
          window->DrawList, bb_inner.Min, bb_inner.Max, GetColorU32(col_source), grid_step, ImVec2(off, off), rounding);
    else
  ...
  if (!is_leaf) {
    if (pressed && g.DragDropHoldJustPressedId != id) {
      ...
      if (flags & ImGuiTreeNodeFlags_OpenOnArrow)
        toggled |= is_mouse_x_over_arrow && !g.NavDisableMouseHover;  // Lightweight equivalent of
    ...
    if (g.NavId == id && g.NavMoveDir == ImGuiDir_Right && !is_open)  // If there's something upcoming on the line we may
...
void ImGui::TreePop() {
  ...
  IM_ASSERT(window->IDStack.Size > 1);  // There should always be 1 element in the IDStack (pushed
...
// ImGuiTreeNodeFlags_CollapsingHeader). You can remove the _NoTreePushOnOpen
// flag if you want behavior closer to normal TreeNode().
bool ImGui::CollapsingHeader(const char *label, ImTextureID *texture, ImGuiTreeNodeFlags flags) {
  ImGuiWindow *window = GetCurrentWindow();
  if (window->SkipItems)
    return false;

  return TreeNodeBehavior(window->GetID(label), texture, flags | ImGuiTreeNodeFlags_CollapsingHeader, label);
}
...
bool ImGui::BeginListBox(const char *label, const ImVec2 &size_arg) {
  ...
  ImVec2 size =
      ImFloor(CalcItemSize(size_arg, CalcItemWidth(), GetTextLineHeightWithSpacing() * 7.25f + style.FramePadding.y * 2.0f));
  ...
  // FIXME-OPT: We could omit the BeginGroup() if label_size.x but would need to
  // omit the EndGroup() as well.
  BeginGroup();
  if (label_size.x > 0.0f) {
...
int ImGui::PlotEx(
  ...
  if (values_count >= values_count_min) {
    ...
    float t0 = 0.0f;
    ImVec2 tp0 = ImVec2(t0, 1.0f - ImSaturate((v0 - scale_min) * inv_scale));  // Point in the normalized space of
    ...
    for (int n = 0; n < res_w; n++) {
      ...
      ImVec2 pos1 = ImLerp(
          inner_bb.Min, inner_bb.Max, (plot_type == ImGuiPlotType_Lines) ? tp1 : ImVec2(tp1.x, histogram_zero_line_t));
      if (plot_type == ImGuiPlotType_Lines) {
...
bool ImGui::BeginMenuBar() {
  ...
  ImRect clip_rect(
      ...
      IM_ROUND(bar_rect.Min.y + window->WindowBorderSize - 12.0f),
...
bool ImGui::BeginBottomBar() {
  ImGuiWindow *window = GetCurrentWindow();
  if (window->SkipItems)
    return false;
  if (!(window->Flags & ImGuiWindowFlags_BottomBar))
    return false;

  IM_ASSERT(!window->DC.BottomBarAppending);
  BeginGroup();  // Backup position on layer 0 // FIXME: Misleading to use a
                 // group for that backup/restore
  PushID("##bottombar");
  ...
  // tend to display over the lower-right rounded area, which looks particularly
  // glitchy.
  ImRect bar_rect = window->BottomBarRect();
  ImRect clip_rect(
      IM_ROUND(bar_rect.Min.x + window->WindowBorderSize),
      IM_ROUND(bar_rect.Min.y + window->WindowBorderSize - 12.0f),
      IM_ROUND(ImMax(bar_rect.Min.x, bar_rect.Max.x - ImMax(window->WindowRounding, window->WindowBorderSize))),
  ...
  // analogous here, maybe a BeginGroupEx() with flags).
  window->DC.CursorPos = window->DC.CursorMaxPos =
      ImVec2(bar_rect.Min.x + window->DC.BottomBarOffset.x, bar_rect.Min.y + window->DC.BottomBarOffset.y);
  window->DC.LayoutType = ImGuiLayoutType_Horizontal;
  window->DC.NavLayerCurrent = ImGuiNavLayer_Menu;
  window->DC.BottomBarAppending = true;
  AlignTextToFramePadding();
  return true;
}

void ImGui::EndBottomBar() {
  ImGuiWindow *window = GetCurrentWindow();
  if (window->SkipItems)
    return;
  ImGuiContext &g = *GImGui;
  ...
  PopClipRect();
  PopID();
  window->DC.BottomBarOffset.x =
      window->DC.CursorPos.x - window->Pos.x;  // Save horizontal position so next append can reuse it.
                                               // This is kinda equivalent to a per-layer CursorPos.
  g.GroupStack.back().EmitItem = false;
  EndGroup();  // Restore position on layer 0
  window->DC.LayoutType = ImGuiLayoutType_Vertical;
  window->DC.NavLayerCurrent = ImGuiNavLayer_Main;
  window->DC.BottomBarAppending = false;
}

bool ImGui::BeginMenuBar(float offsetY) {
  ImGuiWindow *window = GetCurrentWindow();
  if (window->SkipItems)
    return false;
  if (!(window->Flags & ImGuiWindowFlags_MenuBar))
  ...
  // tend to display over the lower-right rounded area, which looks particularly
  // glitchy.
  ImRect bar_rect = window->MenuBarRect(offsetY);
  ImRect clip_rect(
      IM_ROUND(bar_rect.Min.x + window->WindowBorderSize),
...
bool ImGui::BeginViewportSideBar(
  ...
  SetNextWindowViewport(viewport->ID);  // Enforce viewport so we don't create our own viewport
...
static bool IsRootOfOpenMenuSet() {
  ...
  // Initially we used 'OpenParentId' to differentiate multiple menu sets from
  // each others (e.g. inside menu bar vs loose menu items) based on parent ID.
  // This would however prevent the use of e.g. PuhsID() user code submitting
  // menus. Previously this worked between popup and a first child menu because
  // the first child menu always had the _ChildWindow flag, making  hovering on
  // parent popup possible while first child menu was focused - but this was
  // generally a bug with other side effects. Instead we don't treat Popup
  // specifically (in order to consistently support menu features in them),
  // maybe the first child menu of a Popup doesn't have the _ChildWindow flag,
  // and we rely on this IsRootOfOpenMenuSet() check to allow hovering between
  // root window/popup and first chilld menu.
  const ImGuiPopupData *upper_popup = &g.OpenPopupStack[g.BeginPopupStack.Size];
  return (/*upper_popup->OpenParentId == window->IDStack.back() &&*/ upper_popup->Window &&
...
bool ImGui::BeginMenuEx(const char *label, const char *icon, bool enabled) {
  ...
  // accross (otherwise e.g. resizing borders with
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    popup_pos =
        ImVec2(pos.x - 1.0f - IM_FLOOR(style.ItemSpacing.x * 0.5f), pos.y - style.FramePadding.y + window->MenuBarHeight());
    ...
    window->DC.CursorPos.x +=
        IM_FLOOR(style.ItemSpacing.x * (-1.0f + 0.5f));  // -1 spacing to compensate the spacing added when
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Vertical)  // (window->Flags &
                                                          // (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
  {
    ...
    ImGuiWindow *child_menu_window =
        (g.BeginPopupStack.Size < g.OpenPopupStack.Size && g.OpenPopupStack[g.BeginPopupStack.Size].SourceWindow == window)
    ...
    if (g.HoveredWindow == window && child_menu_window != NULL && !(window->Flags & ImGuiWindowFlags_MenuBar)) {
      ...
      tb.y = ta.y + ImMax(
                        (tb.y - extra) - ta.y,
                        -ref_unit * 8.0f);  // triangle is maximum 200 high to limit the
      ...
      // moving_toward_other_child_menu ? IM_COL32(0,128,0,128) :
    ...
    } else if (pressed || (hovered && menuset_is_open && !menu_is_open))  // First click to open, then hover to
                                                                          // open others
    {
      want_open = true;
  ...
  if (menu_is_open) {
    SetNextWindowPos(
        popup_pos,
        ImGuiCond_Always);  // Note: this is super misleading! The value will
    ...
    menu_is_open = BeginPopupEx(id, flags);  // menu_is_open can be 'false' when the popup is completely
...
bool ImGui::BeginMenuImage(const char *label, ImTextureID *texture, bool enabled) {
  return BeginMenuImageEx(label, NULL, texture ? *texture : (ImTextureID)0, enabled);
}

bool ImGui::BeginMenuImageEx(const char *label, const char *icon, ImTextureID texture, bool enabled) {
  ImGuiWindow *window = GetCurrentWindow();
  if (window->SkipItems)
    return false;
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    if (texture != (ImTextureID)0)
      ...
      icon_w = CalcTextSize(icon, NULL).x;
    ...
    if (texture != (ImTextureID)0) {
      ImVec2 texture_size(g.FontSize, g.FontSize);
...
bool ImGui::MenuItemImageEx(const char *label, ImTextureID texture, const char *shortcut, bool selected, bool enabled) {
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    if (texture != NULL) {
      ImVec2 texture_size(g.FontSize,
                          g.FontSize);  // Set size for the texture (e.g., square icon)
      ImVec2 texture_pos = pos + ImVec2(offsets->OffsetIcon, 0.0f);
      window->DrawList->AddImage(texture, texture_pos, texture_pos + texture_size);
    }
...
static void ImGui::TabBarLayout(ImGuiTabBar *tab_bar) {
  ...
  sections[0].Spacing =
      sections[0].TabCount > 0 && (sections[1].TabCount + sections[2].TabCount) > 0 ? g.Style.ItemInnerSpacing.x : 0.0f;
  ...
  if (tab_bar->ScrollingAnim != tab_bar->ScrollingTarget) {
    ...
    const bool teleport =
        (tab_bar->PrevFrameVisible + 1 < g.FrameCount) || (tab_bar->ScrollingTargetDistToVisibility > 10.0f * g.FontSize);
    tab_bar->ScrollingAnim =
        teleport ? tab_bar->ScrollingTarget
                 : ImLinearSweep(tab_bar->ScrollingAnim, tab_bar->ScrollingTarget, g.IO.DeltaTime * tab_bar->ScrollingSpeed);
  } else {
...
// Dockable uses Name/ID in the global namespace. Non-dockable items use the ID
// stack.
...
void ImGui::TabBarAddTab(ImGuiTabBar *tab_bar, ImGuiTabItemFlags tab_flags, ImGuiWindow *window) {
  ...
  IM_ASSERT(g.CurrentTabBar != tab_bar);  // Can't work while the tab bar is active as our tab doesn't
  ...
  if (!window->HasCloseButton)
    tab_flags |= ImGuiTabItemFlags_NoCloseButton;  // Set _NoCloseButton immediately
...
static void ImGui::TabBarScrollToTab(ImGuiTabBar *tab_bar, ImGuiID tab_id, ImGuiTabBarSection *sections) {
  ...
  ImGuiContext &g = *GImGui;
  float margin = g.FontSize * 1.0f;  // When to scroll to make Tab N+1 visible always make a
  ...
  float tab_x2 =
      tab->Offset - sections[0].Width + tab->Width + (order + 1 < tab_bar->Tabs.Size - sections[2].TabCount ? margin : 1.0f);
  ...
  if (select_dir != 0)
    if (ImGuiTabItem *tab_item = TabBarFindTabByID(tab_bar, tab_bar->SelectedTabId)) {
      ...
      while (tab_to_scroll_to == NULL) {
        ...
        // visible
        tab_to_scroll_to =
            &tab_bar->Tabs[(target_order >= 0 && target_order < tab_bar->Tabs.Size) ? target_order : selected_order];
...
bool ImGui::BeginTabItem(const char *label, bool *p_open, ImGuiTabItemFlags flags) {
  ...
  IM_ASSERT((flags & ImGuiTabItemFlags_Button) == 0);  // BeginTabItem() Can't be used with button flags, use
...
bool ImGui::TabItemEx(
  ...
  if (tab->Window != NULL) {
    ...
    tab_bar->TabsNames.append(label,
                              label + strlen(label) + 1);  // Append name _with_ the zero-terminator.
  }
  ...
  if (g.DragDropActive && !g.DragDropPayload.IsDataType(IMGUI_PAYLOAD_TYPE_WINDOW))  // FIXME: May be an opt-in property of
    ...
    if (docked_window != NULL && !(docked_window->Flags & ImGuiWindowFlags_NoMove)) {
      ...
      if (!undocking_tab)  //&& (!g.IO.ConfigDockingWithShift || g.IO.KeyShift)
      {
        ...
        float threshold_y =
            (threshold_base * 1.5f) +
            ImClamp(
                (ImFabs(g.IO.MouseDragMaxDistanceAbs[0].x) - threshold_base * 2.0f) * 0.20f, 0.0f, threshold_base * 4.0f);
  ...
  IM_ASSERT(
      !is_tab_button || !(tab_bar->SelectedTabId == tab->ID && is_tab_button));  // TabItemButton should not be selected
  if (is_tab_button)
    return pressed;
  return tab_contents_visible;
}
...
ImVec2 ImGui::TabItemCalcSize(const char *label, bool has_close_button) {
  ...
  return ImVec2(ImMin(size.x + 45.0f, TabBarCalcMaxTabWidth()), size.y);
}
...
void ImGui::TabItemBackground(ImDrawList *draw_list, const ImRect &bb, ImGuiTabItemFlags flags, ImU32 col) {
  ...
  const float rounding = ImMax(
      0.0f, ImMin((flags & ImGuiTabItemFlags_Button) ? g.Style.FrameRounding : g.Style.TabRounding, width * 0.5f - 1.0f));
...
void ImGui::TabItemLabelAndCloseButton(
  ...
  if (texture != NULL) {
    ...
    ImVec2 texture_pos(
        ...
        bb.Min.y + (bb.GetHeight() - texture_height) * 0.5f);  // Adjust position as needed
  ...
  // Render text label (with clipping + alpha gradient) + unsaved marker
  float extra_offset = (bb.Max.y - bb.Min.y < 25.0f) ? 10.0f : 5.0f;

  ImRect text_pixel_clip_bb(
      bb.Min.x + frame_padding.x * 2.0f + extra_offset + frame_padding.x,
