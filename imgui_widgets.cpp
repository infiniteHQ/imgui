...
bool ImGui::InputColorScalar(const char* label, ImGuiDataType data_type, ImVec4 color, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::InputColorFloat(const char* label, float* v, ImVec4 color, float step, float step_fast, const char* format, ImGuiInputTextFlags flags)
{
    flags |= ImGuiInputTextFlags_CharsScientific;
    return InputColorScalar(label, ImGuiDataType_Float, color, (void*)v, (void*)(step > 0.0f ? &step : NULL), (void*)(step_fast > 0.0f ? &step_fast : NULL), format, flags);
}
...
bool ImGui::InputTextWithImage(const char* label, ImTextureID texture, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
...
bool ImGui::InputTextEx(const char* label, ImTextureID texture, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
  ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
if (render_cursor || render_selection)
{
...

    {
        ...
        cursor_offset.x = InputTextCalcTextSizeW(ImStrbolW(searches_input_ptr[0], text_begin), searches_input_ptr[0]).x + (icon_size + style.ItemInnerSpacing.x);
        cursor_offset.y = searches_result_line_no[0] * g.FontSize;
        if (searches_result_line_no[1] >= 0)
        {
            select_start_offset.x = InputTextCalcTextSizeW(ImStrbolW(searches_input_ptr[1], text_begin), searches_input_ptr[1]).x + (icon_size + style.ItemInnerSpacing.x);
            select_start_offset.y = searches_result_line_no[1] * g.FontSize;
        }
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
            draw_window->DrawList->AddText(g.Font, g.FontSize, ImVec2(draw_pos.x + icon_size, draw_pos.y) - draw_scroll, col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    ...
    else
    {
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
            draw_window->DrawList->AddText(g.Font, g.FontSize, ImVec2(draw_pos.x + icon_size, draw_pos.y), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    }
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImTextureID* texture, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (display_frame)
    {
        ...
        if (flags & ImGuiTreeNodeFlags_Bullet)
            ...
            RenderArrow(window->DrawList, texture, ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y), text_col, is_open ? ImGuiDir_Down : ImGuiDir_Right, 1.0f);
    ...
    else
    {
        ...
        // TESTER AVEC l'ARROW immtexure
            RenderArrow(window->DrawList, texture, ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y + g.FontSize * 0.15f), text_col, is_open ? ImGuiDir_Down : ImGuiDir_Right, 0.70f);
...
bool ImGui::BeginMenuBar()
{
    ...
    ImRect clip_rect(IM_ROUND(bar_rect.Min.x + window->WindowBorderSize), IM_ROUND(bar_rect.Min.y + window->WindowBorderSize - 12.0f), IM_ROUND(ImMax(bar_rect.Min.x, bar_rect.Max.x - ImMax(window->WindowRounding, window->WindowBorderSize))), IM_ROUND(bar_rect.Max.y));
...
bool ImGui::BeginBottomBar()
{
    ...
    // We overwrite CursorMaxPos because BeginGroup sets it to CursorPos (essentially the .EmitItem hack in EndMenuBar() would need something analogous here, maybe a BeginGroupEx() with flags).
    window->DC.CursorPos = window->DC.CursorMaxPos = ImVec2(bar_rect.Min.x + window->DC.BottomBarOffset.x, bar_rect.Min.y + window->DC.BottomBarOffset.y);
    window->DC.LayoutType = ImGuiLayoutType_Horizontal;
    window->DC.NavLayerCurrent = ImGuiNavLayer_Menu;
...
void ImGui::EndBottomBar()
{
    ...
    PopClipRect();
    PopID();
    window->DC.BottomBarOffset.x = window->DC.CursorPos.x - window->Pos.x; // Save horizontal position so next append can reuse it. This is kinda equivalent to a per-layer CursorPos.
