...
inline SaveReasonFlags operator|(SaveReasonFlags lhs, SaveReasonFlags rhs) {
  return static_cast<SaveReasonFlags>(static_cast<uint32_t>(lhs) |
...
using ConfigSaveSettings = bool (*)(const char *data, size_t size,
                                    SaveReasonFlags reason, void *userPointer);
using ConfigLoadSettings = size_t (*)(char *data, void *userPointer);

using ConfigSaveNodeSettings = bool (*)(NodeId nodeId, const char *data,
                                        size_t size, SaveReasonFlags reason,
                                        void *userPointer);
using ConfigLoadNodeSettings = size_t (*)(NodeId nodeId, char *data,
                                          void *userPointer);
...
struct Config {
  ...
  int DragButtonIndex; // Mouse button index drag action will react to (0-left,
                       // 1-right, 2-middle)
  int SelectButtonIndex;   // Mouse button index select action will react to
                           // (0-left, 1-right, 2-middle)
  int NavigateButtonIndex; // Mouse button index navigate action will react to
                           // (0-left, 1-right, 2-middle)
  int ContextMenuButtonIndex; // Mouse button index context menu action will
                              // react to (0-left, 1-right, 2-middle)
  bool EnableSmoothZoom;
  float SmoothZoomPower;

  Config()
      : SettingsFile("NodeEditor.json"), BeginSaveSession(nullptr),
        EndSaveSession(nullptr), SaveSettings(nullptr), LoadSettings(nullptr),
        SaveNodeSettings(nullptr), LoadNodeSettings(nullptr),
        UserPointer(nullptr), CustomZoomLevels(),
        CanvasSizeMode(CanvasSizeModeAlias::FitVerticalView),
        DragButtonIndex(0), SelectButtonIndex(0), NavigateButtonIndex(1),
        ContextMenuButtonIndex(1), EnableSmoothZoom(false)
#ifdef __APPLE__
        ,
...
struct Style {
  ...
  float SnapLinkToPinDir; // when true link will start on the line defined by
                          // pin direction
...
IMGUI_NODE_EDITOR_API EditorContext *
CreateEditor(const Config *config = nullptr);
...
IMGUI_NODE_EDITOR_API void PushStyleColor(StyleColor colorIndex,
                                          const ImVec4 &color);
...
IMGUI_NODE_EDITOR_API void Begin(const char *id,
...
IMGUI_NODE_EDITOR_API bool Link(LinkId id, PinId startPinId, PinId endPinId,
                                const ImVec4 &color = ImVec4(1, 1, 1, 1),
                                float thickness = 1.0f);

IMGUI_NODE_EDITOR_API void
Flow(LinkId linkId, FlowDirection direction = FlowDirection::Forward);

IMGUI_NODE_EDITOR_API bool BeginCreate(const ImVec4 &color = ImVec4(1, 1, 1, 1),
                                       float thickness = 1.0f);
...
IMGUI_NODE_EDITOR_API bool QueryNewNode(PinId *pinId, const ImVec4 &color,
                                        float thickness = 1.0f);
IMGUI_NODE_EDITOR_API bool AcceptNewItem();
IMGUI_NODE_EDITOR_API bool AcceptNewItem(const ImVec4 &color,
                                         float thickness = 1.0f);
IMGUI_NODE_EDITOR_API void RejectNewItem();
IMGUI_NODE_EDITOR_API void RejectNewItem(const ImVec4 &color,
                                         float thickness = 1.0f);
IMGUI_NODE_EDITOR_API void EndCreate();

IMGUI_NODE_EDITOR_API bool BeginDelete();
IMGUI_NODE_EDITOR_API bool QueryDeletedLink(LinkId *linkId,
                                            PinId *startId = nullptr,
                                            PinId *endId = nullptr);
...
IMGUI_NODE_EDITOR_API void SetNodePosition(NodeId nodeId,
...
IMGUI_NODE_EDITOR_API void
SetNodeZPosition(NodeId nodeId,
                 float z); // Sets node z position, nodes with higher value are
                           // drawn over nodes with lower value
IMGUI_NODE_EDITOR_API float
GetNodeZPosition(NodeId nodeId); // Returns node z position, defaults is 0.0f
...
HasAnyLinks(NodeId nodeId); // Returns true if node has any link connected
IMGUI_NODE_EDITOR_API bool
HasAnyLinks(PinId pinId); // Return true if pin has any link connected
IMGUI_NODE_EDITOR_API int
BreakLinks(NodeId nodeId); // Break all links connected to this node
IMGUI_NODE_EDITOR_API int
BreakLinks(PinId pinId); // Break all links connected to this pin

IMGUI_NODE_EDITOR_API void NavigateToContent(float duration = -1);
IMGUI_NODE_EDITOR_API void NavigateToSelection(bool zoomIn = false,
                                               float duration = -1);
...
IMGUI_NODE_EDITOR_API int GetOrderedNodeIds(
    NodeId *nodes,
    int size); // Fills an array with node id's in order they're drawn; up to
               // 'size` elements are set. Returns actual size of filled id's.

//------------------------------------------------------------------------------
namespace Details {

template <typename T, typename Tag> struct SafeType {
...
template <typename Tag> struct SafePointerType : SafeType<uintptr_t, Tag> {
  ...
  template <typename T = void> T *AsPointer() const {
...
template <typename Tag> const Tag SafePointerType<Tag>::Invalid = {0};

template <typename Tag>
inline bool operator==(const SafePointerType<Tag> &lhs,
...
inline const char *ToString(const PinId &id) {
  static thread_local char buffer[32];
  std::snprintf(buffer, sizeof(buffer), "%llu",
                static_cast<unsigned long long>(id.Get()));
  return buffer;
}

inline PinId PinIdFromString(const char *s) {
  return PinId(static_cast<uintptr_t>(std::strtoull(s, nullptr, 10)));
}
